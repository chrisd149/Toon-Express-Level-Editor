...
# easy, try to keep the sum at 100 or similar.
...
def getBodyRarity(bodyIndex):
    ...
    # by subtracting off the lowest weight value, we scale this between 0 and 1
...
def getRarity(dna):
    ...
    #currently, only body type has 'rarity'
...
def getGenderString(dna = None, gender = -1):
    # Pass in either a dna array or a gender value
    assert( (dna != None) or (gender in PetGenders) )
