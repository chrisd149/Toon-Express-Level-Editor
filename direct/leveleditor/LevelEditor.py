...
class LevelEditor(NodePath, DirectObject):
    ...
    def createNewGroup(self, type = 'dna'):
        ...
        if type == 'dna':
            ...
            newDNANode = DNAVisGroup('VisGroup' + repr(self.getGroupNum()))
    ...
    def autoSaveDNADefaultFile(self):
        outputFile = self.outputFile
        if outputFile == None:
            outputFile = self.neighborhood + '_autosave_' + strftime("%H-%M-%S", gmtime()) + '.dna'
        else:
            outputFile = outputFile + '_autosave_' + strftime("%H-%M-%S", gmtime()) + '.dna'
        file = os.path.join(dnaDirectory.toOsSpecific(), outputFile)
        self.outputDNA(file)
    ...
    def selectSleepTime(self):
        global sleep_time
        sleep_time = eval(input('Select the time in between auto-saves in seconds:'))
...
class LevelEditorPanel(Pmw.MegaToplevel):
    def __init__(self, levelEditor, parent = None, **kw):
        ...
        INITOPT = Pmw.INITOPT
        optiondefs = (
            ('title',       'ToonExpress Level Editor',       None),
            )
